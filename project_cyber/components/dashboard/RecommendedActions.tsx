...
export default function RecommendedActions({ onViewAll }: RecommendedActionsProps) {
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      case 'critical':
        return 'border-critical text-critical hover:bg-red-50'
      case 'high':
        return 'border-warning text-warning hover:bg-amber-50'
      case 'medium':
        ...
        {actions.map((action, index) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-xs font-medium text-left flex-1">{action.label}</span>
            </button>
          )
        })}
      </div>
