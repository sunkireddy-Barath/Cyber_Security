export default function DepartmentHeatmap() {
  ...
  return (
    <div className="glass border border-white/10 rounded-xl p-4 hover:border-orange-500/30 transition-all group relative overflow-hidden">
      ...
      <h3 className="text-base font-semibold text-black mb-3 relative z-10">Department Risk Heatmap</h3>
      <div className="space-y-2.5 relative z-10">
        {departments.map((dept, index) => (
          <div key={index} className="flex items-center space-x-2">
            <div className="w-24 text-xs font-medium text-black truncate">
              {dept.name}
            </div>
            <div className="flex-1 bg-gray-700/30 rounded-full h-7 relative overflow-hidden border border-gray-600/30">
              <div
            ...
            <div className="w-16 text-xs text-black text-right">
              {dept.incidents} inc
            </div>
          </div>
        ))}
